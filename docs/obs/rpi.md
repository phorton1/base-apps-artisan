...
In the end I changed it back (with better structure and comments)
after proving it was the XDG_RUNTIME_DIR environment variable.


## XDG_RUNTIME_DIR

It was the effing XDG_RUNTIME_DIR variable.
When properly set in /etc/environment (with quotes), and
used in (what is now) linuxAudio pactl commands, it works.
