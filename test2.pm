...
sub doAction
{
	my ($ip,$port,$control_url,$urn,$service,$action,$args) = @_;
    ...
    my $request = '';
    $request .= "POST $control_url HTTP/1.1\r\n";
    $request .= "HOST: $ip:$port\r\n";
    $request .= "Content-Type: text/xml; charset=\"utf-8\"\r\n";
    $request .= "Content-Length: ".length($xml)."\r\n";
    $request .= "soapaction: \"urn:$urn:service:$service:1#$action\"\r\n";
    $request .= "\r\n";
    $request .= $xml;
    ...
    if (!$length && $headers{transfer_encoding} && $headers{transfer_encoding} eq 'chunked')
    {
        my $hex = <$sock>;
        $hex =~ s/^\s*//g;
        $hex =~ s/\s*$//g;
...
my $ip = '192.168.0.104';
my $port = 58645;
my $uuid = '38875023-0ca8-f211-ffff-fffffbda5e0c';
...
{
    my ($ip,$port,$url,$service,$action,$args) = @_;
    
	my $dbg_ren = -2;
	display(0,0,"$action($args->{Target})") if $action =~ /Seek/;
	
	display($dbg_ren+1,0,"private_doAction($action)");
...
if (1)
{
	my $urn = 'schemas-upnp-org';
	my $url = '/dev/38875023-0ca8-f211-ffff-ffffa3c996b8/svc/upnp-org/AVTransport/action';
	# my $url = 'http://192.168.0.100:58645/dev/38875023-0ca8-f211-ffff-ffffa3c996b8/svc/upnp-org/AVTransport/action';
	my $data = undef;
	
	if (1)
	{
		$data= private_doAction(
			"192.168.0.100",
			58645,
			$url,
			"AVTransport",
...
{
	my $urn = "av-openhome-org";
	my $url = 'http://192.168.0.100:58645/dev/38875023-0ca8-f211-0000-000002bdac1d/svc/av-openhome-org/Playlist/action';
	my $data = doAction(
		"192.168.0.100",
		58645,
		$url,
