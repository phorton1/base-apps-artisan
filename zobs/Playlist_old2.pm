...
sub start
{
	...
	display($dbg_pl,0,"Playlist.start($this->{name}) path=$db_path");
	...
	# Implemented as a write-thru cache in conjunction with save()
{
	...
	if (!$this->{track_ids})
	{
		...
		display($dbg_pl,0,"getTrackID() found ".scalar(@$tracks)." items in Playlist($this->{name})");
...
sub sortPlaylist
{
	...
	display($dbg_pl,0,"sortPlaylist($name)");
	...
	for my $track (@$tracks)
	{
		# display($dbg_pl,0,"after sorting position($position)=$track->{title}");
	...
	display($dbg_pl,0,"sortPlaylist($name) finished");
}
...
{
	my ($this) = @_;
	display($dbg_pl,0,"save Playlist($this->{name})");
...
{
	my ($dbh,$table) = @_;
	display($dbg_pl,0,"create_table($table)");
	...
	# set track_index to 0 / 1 as needed
{
	...
	display($dbg_pl,1,"found $this->{num_tracks} tracks");
	$this->fix_track_index();
	...
	# which it should not in my initial usage.
{
    ...
    for my $path (@paths)
    {
		...
		if ($exclude)
		{
			display($dbg_pl,2,"exclude='$exclude'");
			push @$args,$exclude;
	...
	display($dbg_pl,0,"inserting ".scalar(@$tracks)." items in new Playlist($this->{name})");
...
{
	display($dbg_pl,0,"static_init_playlists started ...");
	my $new_database = ! -f "$data_dir/playlists.db";
	my $dbh = sqlite_connect("$data_dir/playlists.db",'playlists','');
	...
	display($dbg_pl,1,"check/create default playlists");
	for my $name (keys(%default_playlists))
	{
		my $playlist = get_record_db($dbh,
			"SELECT * FROM playlists WHERE name='$name'");

		display($dbg_pl,1,"got($name)="._def($playlist));

		if (!$playlist)
		{
			display($dbg_pl,1,"creating new playlist($name) from the default");
		...
		elsif (-f "$playlist_dir/$name.db")
		{
			...
			if (0)
			{
				display($dbg_pl,1,"getting playlist($name) from existing databse");
		...
		{
			display($dbg_pl,1,"recreating playlist($name) tracks from query");
			bless $playlist,"Playlist";
	...
	display($dbg_pl,1,"got ".scalar(@$recs)." playlists");
	for my $rec (@$recs)
	{
	...
	display($dbg_pl,0,"static_init_playlists finished");
}
