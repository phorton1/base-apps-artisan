	...
	#   'pause'
{
	...
	display($dbg_dlna_ren,0,"doCommand($command,$arg)");
    ...
    # a subtle distinction.
{
    ...
    display($dbg_dlna_ren+2,0,"Status Info\n$data");
    ...
    display($dbg_dlna_ren+2,0,"getState=$state");
    return $state;
}
    ...
    # a hash with the results.
{
    ...
    display($dbg_dlna_ren+2,0,"Position Info\n$data");
    ...
    display($dbg_dlna_ren+1,0,"GOT_POSITION: position=$retval{position} duration=$retval{duration} id=$retval{song_id}");
    display($dbg_dlna_ren+2,1,"uri='$retval{uri}' type='$retval{type}'");
	...
	else
    {
        ...
        if ($data)
        {
            display($dbg_dlna_ren,0,"GOT VOLUME:\n".Dumper($data));
        }
		$data = $this->private_doAction(1,'GetMute');
		if ($data)
		{
			display($dbg_dlna_ren,0,"GOT MUTE:\n".Dumper($data));
		}
    }
	...
	# returns undef in error cases
{
	...
	display($dbg_dlna_ren+1,0,"private_doAction($rv,$action)");
    ...
    display($dbg_dlna_ren+2,1,"sending action($action) request");
    display($dbg_dlna_ren+2,1,"--------------- request --------------------");
    display($dbg_dlna_ren+2,1,$request);
    display($dbg_dlna_ren+2,1,"--------------------------------------------");
    ...
    display($dbg_dlna_ren+1,1,"getting action($action) response");
    ...
    while (defined($line) && $line ne "\r\n")
    {
        chomp($line);
        $line =~ s/\r|\n//g;
        display($dbg_dlna_ren+1,2,"line=$line");
        if ($line =~ /:/)
        {
    ...
    display($dbg_dlna_ren+1,2,"content_length=$length");
    ...
    if (!$length && $headers{transfer_encoding} eq 'chunked')
    {
        ...
        $hex =~ s/\s*$//g;
        $length = hex($hex);
        display($dbg_dlna_ren+1,2,"using chunked transfer_encoding($hex) length=$length");
    }
    ...
    display($dbg_dlna_ren+1,2,"got "._def($rslt)." bytes from socket");

    display($dbg_dlna_ren+1,2,"--------------- response --------------------");
    display($dbg_dlna_ren+1,2,"'$data'");
    display($dbg_dlna_ren+1,2,"--------------------------------------------");
...
# getDLNARenderers
#------------------------------------------------------------
# PRH 2023-11-07 BAD DUPLICATE METHOD NAME
#
#    This one seems to be a list of all known DLNA Renderers,
#       including the local renderer, and probably constructs
#       them as such
#    The one in SSDPSearch returns an unblessed hash of hashes.
