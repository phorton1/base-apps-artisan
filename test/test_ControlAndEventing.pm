#!/usr/bin/perl
#---------------------------------------
# A test one sided control point.
#
...
sub getPositionInfo
{
    if (!$comm_sock)
    {
        display(0,0,"creating comm_sock");
        $comm_sock = IO::Socket::INET->new(
            # LocalAddr => $server_ip,
            # LocalPort => 8078,
            PeerAddr => $bubbleUp_ip,
            PeerPort => $bubbleUp_port,
        ...
        if (!$comm_sock)
        {
            error("Could not create comm_sock");
            exit 1;
        }
    }
    ...
    $request .= "SOAPACTION: \"urn:schemas-upnp-org:service:AVTransport:1#GetPositionInfo\"\r\n";
    $request .= "\r\n";
    $request .= $body;    

    # send the request

    display(0,1,"sending position request");

    $comm_sock->send($request);

    display(0,1,"getting position response");

    my $data = '';
    my $remote = $comm_sock->recv($data,1024);
    my ($peer_port, $peer_addr) = sockaddr_in($remote);
	my $peer_ip = inet_ntoa($peer_addr);

    display(0,1,"peer_addr=$peer_ip:$peer_port");
    display(0,1,"data=$data");
    display(0,1,"done with position request");
}
...
sub old_getPositionInfo
    # Use GetPositionInfo action to get the duration and current
    # position of the playback .. this is slow and inaccurate,
    # taking 5 seconds per call, and returning the stale result.
    # Note that you can also use the GetTransportInfo action to
    # get the state and status of the transport.
{
	my ($avtrans_service) = shift;
    display(1,0,"getTransportInfo()");
    
    my $arg = { InstanceID => 0 };
	my $action_result = $avtrans_service->postaction("GetPositionInfo",$arg);
    my $arg_list = $action_result->getargumentlist();

    if (0)
    {
        for my $k (sort(keys(%$arg_list)))
        {
            display(0,1,pad($k.'=',20).$arg_list->{$k});
        }
    }
    
    display(0,0,pad($arg_list->{RelTime},8).' / '.pad($arg_list->{TrackDuration},8)."  $arg_list->{TrackURI}");
}
...
while ($count--)
{
    ...
    while (time() < $time + 30)
    {
        getPositionInfo($avtrans_service);
    }
}
